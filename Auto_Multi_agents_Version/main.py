...
from src.ai.agent_orchestrator import AgentOrchestrator  # Legacy single-agent
    ...
    async def generate_tests(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            if config.USE_MULTI_AGENT:
                logger.info("🤖 Generating test scenarios with Multi-Agent AI Workflow...")
                orchestrator = MultiAgentOrchestrator()
            else:
                logger.info("🤖 Generating test scenarios with Single-Agent AI...")
            ...
            mode = "multi-agent workflow" if config.USE_MULTI_AGENT else "single-agent"
            logger.info(f"✓ Generated {len(raw_scenarios)} scenarios using {mode}")
